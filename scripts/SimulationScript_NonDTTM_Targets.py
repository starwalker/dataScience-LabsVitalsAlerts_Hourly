...
pred_cols = ['MetSIRS4_4hr_24_dttm_preds']
...
## this chunk of code finds a 90%CI, and the median for the number of unique patients that have the target_col
...
#tmp_df has dimension of (num_dates,num_models) to keep track of how many unique patients had the target_col==1 on each of the date
date_cols = [date_col,week_col]
...
for date_col_ind in range(len(date_cols)):
    ...
    #calculation_df is a tmp variable to store how many times per day/week that the target_col is populated.
    # i.e. when looking at calculating the [' 5%',' 50%',' 95%'] percentiles for targets/day, if there are 100
    # unique days in the dataset, calculation_df will have dimension (100,1)
    all_dates = data[date_col].unique()
    calculation_df = np.zeros((len(all_dates),len(target_cols)))

    #iterate over all the unique dates in the dataset
    for date_ind in range(len(all_dates)):
        date = all_dates[date_ind]
        #create a tmp_dataset that only contains the data with the current date we are looking at
        ...
        for target_ind in range(len(target_cols)):
            ...
            ## find the length of the vector containing unique_keys where the target_col==1
            calculation_df[date_ind,target_ind] = len(np.unique(tmp_data[key_col][np.where(tmp_data[target_col]==1)[0]]))

    # if there are multiple target columns that we are looking at iterate over them.
    # this logic finds the 5,50,95 percentile values for the number of unique keys with the target_col==1
    for target_ind in range(len(target_cols)):
        target_col = target_cols[target_ind]
        targets_per_date_df.iloc[0,(date_col_ind*3):((date_col_ind+1)*3)] = np.percentile(calculation_df[:,target_ind],np.array([5,50,95]))

# set the targets_per_date_df.columns
...
for cutoff in cutoffs:
    ...
    # find the keys of the patients who were alerted on at the current cutoff
    unique_alert_keys = data.loc[np.where(data['preds_binary']==1)[0],key_col].unique()

    #iterate over the patients that had the target and determine whether they were alerted on prior, during/after, not at all.
    for target_key in unique_target_keys:
        # calculate their time 0 for the target (minimum time the patient had SIRS4_4hr for example)
        first_target_time = min(data.loc[np.where((data[key_col]==target_key) & (data['MetSIRS4_4hr']==1))[0],time_col])
        # find the indices where the model would have alerted on this patient
        ...
        # ELSE, we alerted on the patient and need to find the time that we would have first alerted on them
        else:
            # calculate the min alert time
            first_alert_time = min(data.loc[alert_inds, time_col])
            # calculate the timedelta between the first alert time and their t0 of the target (in hours). positive values
            # indicate that the model alerted prior, negative that the model alerted after.
            alert_diff = (first_target_time.to_pydatetime() - first_alert_time.to_pydatetime()).total_seconds() / 3600

            # IF alert_time < target_time, the model alerted prior for this patient
            if first_alert_time<first_target_time:
                alerts_dict['Proportion Alerts Prior']+=1
                ## bin the number of hours that we alerted prior
                if alert_diff<=4:
            ...
            ## the model didnt alert prior for this patient
            else:
                alerts_dict['Proportion Alerts During/After'] += 1
                alert_time_bins.loc[cutoff, 'Alerted During/After'] += 1
            # add in the alert diff to the overall average alert time prior.
            alerts_dict['Average Alert Time Before Target(Hours)'] += alert_diff

    # Calculate the proportion of patients that were alerted on, but never had the target_col
    ...
    # calculate proportions for each of the following fields by dividing the the number of unique patients with the target_key
...
## this next bit of code aims calculates the number of unique patient alerts per department using the optimal cutoff
